...
export async function registerRoutes(httpServer: Server, app: Express) {
  ...
  app.get("/api/groups/:id/progress", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const groupId = parseInt(req.params.id);
      ...
      const progress = await storage.getGroupMemberProgress(groupId);
      res.json(progress);
    } catch (error) {
  ...
  app.patch("/api/groups/:groupId/habits/:habitId", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      ...
      const { name, category, weeklyGoal, daysOfWeek } = req.body;
      const updated = await storage.updateGroupHabit(habitId, { name, category, weeklyGoal, daysOfWeek });
      if (!updated) {
