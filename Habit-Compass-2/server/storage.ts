...
export interface IStorage {
  ...
  updateGroup(groupId: number, data: { name?: string; description?: string }): Promise<Group | undefined>;
  ...
  updateGroupHabit(habitId: number, data: { name?: string; category?: string; weeklyGoal?: number; daysOfWeek?: number[] }): Promise<GroupHabit | undefined>;
  deleteGroupHabit(habitId: number): Promise<void>;
  
  getGroupCompletions(groupId: number): Promise<GroupHabitCompletion[]>;
  getGroupMemberProgress(groupId: number): Promise<{ members: (GroupMember & { user?: User })[], habits: GroupHabit[], completions: GroupHabitCompletion[] }>;
...
class DatabaseStorage implements IStorage {
  ...
  async updateGroup(groupId: number, data: { name?: string; description?: string }): Promise<Group | undefined> {
  ...
  async updateGroupHabit(habitId: number, data: { name?: string; category?: string; weeklyGoal?: number; daysOfWeek?: number[] }): Promise<GroupHabit | undefined> {
    const [updated] = await db.update(groupHabits).set(data).where(eq(groupHabits.id, habitId)).returning();
    return updated;
  }
  ...
  async getGroupMemberProgress(groupId: number): Promise<{ members: (GroupMember & { user?: User })[], habits: GroupHabit[], completions: GroupHabitCompletion[] }> {
    const members = await this.getGroupMembers(groupId);
    const habits = await this.getGroupHabits(groupId);
    const completions = await this.getGroupCompletions(groupId);
    return { members, habits, completions };
  }
